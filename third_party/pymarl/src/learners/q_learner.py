...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # Optional: CSV logging of learner stats (loss etc.)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.learner_csv_path is not None:
            ...
            td_abs = (masked_td_error.abs().sum().item() / max(mask_elems, 1e-6))
            q_taken_mean = (chosen_action_qvals * mask).sum().item() / (max(mask_elems, 1e-6) * self.args.n_agents)
            target_mean = (targets * mask).sum().item() / (max(mask_elems, 1e-6) * self.args.n_agents)
    ...
    def save_weights(self, path=None):
            """
            Weights-only checkpoint (easy eval):
            - mac weights
            - mixer weights (if any)
            No optimizer.
            """
            if path is None:
                path = getattr(self.args, "model_dir", None) or "models"
            ...
            # mac weights (agent networks)
            self.mac.save_models(path)

            # mixer weights
            if self.mixer is not None:
                th.save(self.mixer.state_dict(), os.path.join(path, "mixer.th"))
