...
from eval_fdqn import run_eval
...
def summarize_config(fdqn_root: str, config: str):
    raw_cfg = config[len("FDQN_"):] if config.startswith("FDQN_") else config
    cfg_dir = os.path.join(fdqn_root, "evaluation", f"FDQN_{raw_cfg}")
    ...
    across = {"config": raw_cfg, "n_models": int(len(model_summary_df))}
    for m in metrics:
        col = f"{m}_mean"
        ...
        if len(vals) > 1:
            ...
            std = 0.0
            se = 0.0
        across[f"{m}_across_models_std"] = std
        across[f"{m}_across_models_se"] = se
...
def main():
    ...
    # FDQN root is the folder containing this script
    ...
    for cfg in args.configs:
        ...
        summarize_config(FDQN_ROOT, cfg)
